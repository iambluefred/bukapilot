...
class CarState(CarStateBase):
  ...
  def update(self, cp):
    ...
    # Todo
    ret.doorOpen = False #any([cp.vl["DOOR_LEFT_SIDE"]['BACK_LEFT_DOOR'],
    ...
    self.cruise_speed = int(cp.vl["ACC_HUD_ADAS"]['SET_SPEED']) * CV.KPH_TO_MS
    ret.cruiseState.speedCluster = self.cruise_speed
    ret.cruiseState.speed = ret.cruiseState.speedCluster / HUD_MULTIPLIER
    ret.cruiseState.standstill = False # Todo bool(cp.vl["ACC_CMD"]["STANDSTILL2"])
    ...
    ret.genericToggle = False # Todo bool(cp.vl["LEFT_STALK"]["GENERIC_TOGGLE"])

    ret.espDisabled = False # Todo bool(cp.vl["PARKING_BRAKE"]["ESC_ON"]) != 1

    # blindspot sensors
    if self.CP.enableBsm:
      # used for lane change so its okay for the chime to work on both side.
      ret.leftBlindspot = bool(cp.vl["BSM"]["LEFT_APPROACH"])
      ret.rightBlindspot = False # Todo bool(cp.vl["BSM"]["RIGHT_APPROACH"])
