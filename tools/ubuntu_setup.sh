...
BP_ROOT=$(git rev-parse --show-toplevel)
...
# in the bukapilot repo
cd $BP_ROOT

source ~/.bashrc
if [ -z "$BUKAPILOT_ENV" ]; then
  printf "\nsource %s/tools/bukapilot_env.sh" "$BP_ROOT" >> ~/.bashrc
  source ~/.bashrc
  echo "added bukapilot_env to bashrc"
fi
...
# install python
PYENV_PYTHON_VERSION=$(cat $BP_ROOT/.python-version)
